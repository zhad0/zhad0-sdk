...
| `simulateProof()` SHA-256 stand-in for RISC Zero | MAINNET_PENDING |
| `submitIntent()` returns `txHash: null` | MAINNET_PENDING |
...
console.log(receipt.txHash);       // null (mainnet pending)
```
...
Validates the intent with Zod, canonicalises to deterministic JSON, generates an AES-256-GCM key (cached per client), encrypts with a random 96-bit IV, returns:
...
Encrypts, simulates the proof, returns a receipt with `txHash: null` until mainnet is live.
