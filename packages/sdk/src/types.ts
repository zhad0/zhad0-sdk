import { z } from "zod";
import type { ZkIntentProof } from "@zhad0/proof";
...
export type { ZkIntentProof } from "@zhad0/proof";

export interface SubmitReceipt {
  status: "PROVEN_OK" | "PROVEN_INVALID";
  intentHash: string;
  /** Real Schnorr non-interactive zero-knowledge proof of intent validity. */
  proof: ZkIntentProof;
  /** Result of locally re-verifying the proof before relay. */
  verified: boolean;
  proveMs: number;
  /** On-chain anchoring is operator-gated; activates at mainnet launch. */
  txHash: null;
  notice: string;
}
...
export const SDK_STATUS = {
  ...
  message:
    ...
    "(Schnorr non-interactive proof over secp256k1, independently verifiable). " +
